...
public class SillyOdditiesItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        renderNothing(stack, displayContext, poseStack, buffer, packedLight, packedOverlay);
    }

    public void renderNothing(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        if (stack.getItem() == SillyOdditiesItems.NOTHING.get()) {
            ItemStack item = NothingItemModel.getNothingItem();

            poseStack.pushPose();
            poseStack.translate(0.5f, 0.5f, 0.5f);
            Minecraft.getInstance().getItemRenderer().renderStatic(item, displayContext, packedLight, packedOverlay, poseStack, buffer, null, 0);
            poseStack.popPose();
        }
    }
}
